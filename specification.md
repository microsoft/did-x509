...
2. Check whether the list of certificates form a valid certificate chain using the [RFC 5280 certification path validation](https://www.rfc-editor.org/rfc/rfc5280#section-6) procedures with the last certificate in the chain as trust anchor. After performing RFC 5280 certification path validation, additionally fail if any certificate in the chain contains a critical extension that is neither (a) one of the extensions represented in the JSON data model (`extendedKeyUsage`, `subjectAltName`, `fulcio_issuer`), nor (b) one of the standard extensions that RFC 5280 certification path validation processes (which includes `basicConstraints`, `keyUsage`, `nameConstraints`, `policyConstraints`, `policyMappings`, `certificatePolicies`, and `inhibitAnyPolicy`). A conforming implementation MUST accept a critical `fulcio_issuer` extension, even though a generic RFC 5280 validator would reject it as an unrecognized critical extension.
